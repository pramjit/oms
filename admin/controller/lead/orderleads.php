<?php
class ControllerLeadOrderleads extends Controller {
	private $error = array();
	...
	public function delete() {
		...
		if (isset($response['success'])) {
			...
			$this->response->redirect($this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['invoice'] = $this->url->link('lead/orderleads/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$data['shipping'] = $this->url->link('lead/orderleads/shipping', 'token=' . $this->session->data['token'], 'SSL');
		$data['insert'] = $this->url->link('lead/orderleads/add', 'token=' . $this->session->data['token'], 'SSL');
...
$filter_data = array(
		...
		$order_total = $this->model_lead_orderleads->getTotalOrders($filter_data);

		$results = $this->model_lead_orderleads->getOrders($filter_data);

		foreach ($results as $result) { //print_r($result);

if($result['date_potential']=="0000-00-00 00:00:00")
{
$date_potential_temp="00/00/0000";
}
else
{
$date_potential_temp=date($this->language->get('date_format_short'), strtotime($result['date_potential']));
}
			$data['orders'][] = array(
				...
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
                                'date_potential'=> $date_potential_temp,
				'shipping_code' => $result['shipping_code'],
				'view'          => $this->url->link('lead/orderleads/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'edit'          => $this->url->link('lead/orderleads/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'delete'        => $this->url->link('lead/orderleads/delete', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_order'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=o.date_modified' . $url, 'SSL');
                $data['sort_date_potential'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . '&sort=o.date_potential' . $url, 'SSL');
		$url = '';
		...
		$this->response->setOutput($this->load->view('lead/order_list_leads.tpl', $data));
	}
	...
	public function getForm() {
		...
		$data['cancel'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$order_info = $this->model_lead_orderleads->getOrder($this->request->get['order_id']);
		}
		...
		if (!empty($order_info)) {
			...
			if (isset($this->session->data['cookie'])) {
				$products = $this->model_lead_orderleads->getOrderProducts($this->request->get['order_id']);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_lead_orderleads->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
			...
			if (isset($this->session->data['cookie'])) {
				$vouchers = $this->model_lead_orderleads->getOrderVouchers($this->request->get['order_id']);
			...
			$order_totals = $this->model_lead_orderleads->getOrderTotals($this->request->get['order_id']);
		...
		$this->response->setOutput($this->load->view('lead/order_form_leads.tpl', $data));
	}
	...
	public function info() {
		...
		$order_info = $this->model_lead_orderleads->getOrder($order_id);
		...
		if ($order_info) {
			...
			$data['shipping'] = $this->url->link('lead/orderleads/shipping', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['invoice'] = $this->url->link('lead/orderleads/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['edit'] = $this->url->link('lead/orderleads/edit', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['cancel'] = $this->url->link('lead/orderleads', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			if ($order_info['customer_id']) {
				$data['customer'] = $this->url->link('lead/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$data['customer'] = '';
			}
			...
			foreach ($vouchers as $voucher) {
				$data['vouchers'][] = array(
					...
					'href'        => $this->url->link('lead/voucher/edit', 'token=' . $this->session->data['token'] . '&voucher_id=' . $voucher['voucher_id'], 'SSL')
				);
			}

			$totals = $this->model_lead_orderleads->getOrderTotals($this->request->get['order_id']);
			...
			// Fraud
			$this->load->model('lead/fraudleads');

			$fraud_info = $this->model_lead_fraudleads->getFraud($order_info['order_id']);
			...
			$this->response->setOutput($this->load->view('lead/order_info_leads.tpl', $data));
		} else {
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'lead/orderleads')) {
	...
	public function createInvoiceNo() {
		...
		if (!$this->user->hasPermission('modify', 'lead/orderleads')) {
			...
			$invoice_no = $this->model_lead_orderleads->createInvoiceNo($order_id);
	...
	public function history() {
		...
		$results = $this->model_lead_orderleads->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this->model_lead_orderleads->getTotalOrderHistories($this->request->get['order_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('lead/orderleads/history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('lead/order_history_leads.tpl', $data));
	}
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$products = $this->model_lead_orderleads->getOrderProducts($order_id);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_lead_orderleads->getOrderOptions($order_id, $product['order_product_id']);
				...
				$vouchers = $this->model_lead_orderleads->getOrderVouchers($order_id);
				...
				$totals = $this->model_lead_orderleads->getOrderTotals($order_id);
		...
		$this->response->setOutput($this->load->view('lead/order_invoice_leads.tpl', $data));
	}
	...
	public function shipping() {
		...
		$this->load->model('sale/orderleads');
		...
		$this->response->setOutput($this->load->view('lead/order_shipping_leads.tpl', $data));
	}
...
$results = $this->model_lead_orderleads->getOrders($filter_data);
    ...
    { 
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $data['shipping_code']);
         ...
         $data['products'] = array();
			...
			$products = $this->model_lead_orderleads->getOrderProducts($data['order_id']);
                        $product_info_string="";
			foreach ($products as $product) {
				$option_data = array();

				$options = $this->model_lead_orderleads->getOrderOptions($data['order_id'], $product['order_product_id']);
    ...
    header('Content-Disposition: attachment;filename="order_leads_report_'.date('dMy').'.xls"');
