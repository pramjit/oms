...
error_reporting(0);
class ControllerManageorderorderlist extends Controller{
	...
	{            
            ...
            $this->load->model('manageorder/orderlist');
            $data['sap'] = $this->model_manageorder_orderlist->getsap();
            $data['wsp'] = $this->model_manageorder_orderlist->getwspname();
            $data['prodname']= $this->model_manageorder_orderlist->prodname($this->request->post);
            ...
            $data['order_list'] = $this->model_manageorder_orderlist->getFilterList();
            
            
        $total_orders = $this->model_manageorder_orderlist->getTotalOrders();
        ...
        $this->response->setOutput($this->load->view('manageorder/orderlist.tpl', $data));
	}
	public function autocomplete() {
		$json = array();

		if (isset($this->request->get['filter_name_sap']) || isset($this->request->get['filter_model'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');

			if (isset($this->request->get['filter_name_sap'])) {
				$filter_name = $this->request->get['filter_name_sap'];
			} else {
				$filter_name = '';
			}
			...
			$filter_data = array(
				...
				'limit'        => $limit
                        ...
                        $this->load->model('manageorder/orderlist');
		        $results = $this->model_manageorder_orderlist->getsap($filter_data);
			foreach ($results as $result) { //print_r($result);
				$option_data = array();

				$json[] = array(
					'user_id' => $result['sap_ref'],
					'sap_ref' => strip_tags(html_entity_decode($result['sap_ref'], ENT_QUOTES, 'UTF-8')),
		...
		if (isset($this->request->get['filter_name_wh']) || isset($this->request->get['filter_model'])) {
			...
			$filter_data = array(
				...
				'limit'        => $limit
                        ...
                        $this->load->model('manageorder/orderlist');
		        $results = $this->model_manageorder_orderlist->getwspname($filter_data);
			foreach ($results as $result) { //print_r($result);
				$option_data = array();
		...
		$this->response->setOutput(json_encode($json));
          ...
          public function autocompletewsap() {
		$json = array();
		...
		if (isset($this->request->get['filter_name_sap']) || isset($this->request->get['filter_model'])) {
			...
			$filter_data = array(
				...
				'limit'        => $limit
                        ...
                        $this->load->model('saleordergenerate/sordergenerate');
		        $results = $this->model_saleordergenerate_sordergenerate->getsap($filter_data);
			foreach ($results as $result) { //print_r($result);
				$option_data = array();
				...
				$json[] = array(
					...
					'username'  => strip_tags(html_entity_decode($result['username'], ENT_QUOTES, 'UTF-8')),
					'sap_ref'   => $result['sap_ref']." (".strip_tags(html_entity_decode($result['sap_ref'], ENT_QUOTES, 'UTF-8')).")",
					'option'    => $option_data,
					'price'     => $result['price']
				);
			}
	...
	/*-----------------------------insert receive order function ends here-----------------*/
        ...
        { 
           // echo 'okkk';//exit;
            ...
            $prolist= $this->model_manageorder_orderlist->deleteprod($this->request->get['id'],$this->request->get['product_id']);
            print_r($prolist);
        }
        ...
        { 
            ...
            $this->load->model('manageorder/orderlist');
            //echo $indid=$this->request->post['indid'];
            $prolist["alldata"]= $this->model_manageorder_orderlist->getProductList($this->request->post['indid']);
            //print_r($prolist);
            
            $tot=count($prolist["alldata"]);
           /* echo "<div class='form-group'>";
            echo "<label class='col-md-3 control-label' for='textinput'>PRODUCT</label> ";
            echo "<label class='col-md-3 control-label' for='textinput'>QUANTITY</label> ";
            echo "<label class='col-md-3 control-label' for='textinput'>WHOLESALER</label> ";
            echo "<label class='col-md-3 control-label' for='textinput'>ADD NEW / REMOVE</label> ";
            echo "</div>";*/
            //******************TABLE HEADING SET *************************//
            echo "<div class='form-group table-responsive'>";
            echo "<table cellpadding='1' cellspacing='1' class='table' id='table_add_product'>";
            echo "<thead>";
            echo "<tr>";
            
            echo "<th >PRODUCT NAME</th>";
            echo "<th >QNT</th>";
          // echo "<th class='text-center'>WHOLESALER</th>";         
            echo "<th >ACTION</th>";
           // echo "<th >Remove</th>";
            echo "</tr>";
            echo "</thead>";
            ...
            for($i=0;$i<$tot;$i++)
            {
              ...
              {
                echo "<tr>";
                echo "<td><input type='text'  readonly='true' name='prod_name[]' value='".$prolist['alldata'][$i]['PRO_NAME']."' id='trans_name$i' class='form-control' required='required'><input type='hidden' id='PRO_ID' name='PRO_ID[]' value='".$prolist['alldata'][$i]['PRO_ID']."' id='trans_name$i' class='form-control' required='required'><input type='hidden' id='ORDER_ID' name='ORDER_ID' value='".$prolist['alldata'][$i]['IND_ID']."'  class='form-control' required='required'></td>";
                echo "<td><input type='text' size='1' name='qty[]' value='".$prolist['alldata'][$i]['PRO_QTY']."' id='trans_name' class='form-control' required='required'></td>";
               // echo "<td><input type='text' name='wholeseller[]' value='".$prolist['alldata'][$i]['wholeseller']."' id='trans_name' class='form-control' required='required'></td>";
              // echo "<th></th>";
                echo "<td class='col-md-1'><button type='button'  class='btn btn-success btn-lg' id='bt_add_new0' onclick='add_new_row(0)'>+</button></td>";
                       
                echo "<tr>";
              }else
              {
                 echo "<tr>";
                echo "<td><input type='hidden' id='PRO_ID' name='PRO_ID[]' value='".$prolist['alldata'][$i]['PRO_ID']."' id='trans_name$i' class='form-control' required='required'><input type='hidden' id='ORDER_ID' name='ORDER_ID' value='".$prolist['alldata'][$i]['IND_ID']."'  class='form-control' required='required'><input type='text' readonly='true' name='prod_name[]' value='".$prolist['alldata'][$i]['PRO_NAME']."' id='trans_name$i' class='form-control' required='required'><input type='hidden' value='".$tot."' id='Totalnuwrows' name='totalrows' ></td>";
                echo "<td><input type='text' size='5' name='qty[]' value='".$prolist['alldata'][$i]['PRO_QTY']."' id='trans_name' class='form-control' required='required'></td>";
                //echo "<td><input type='text' name='wholeseller[]' value='".$prolist['alldata'][$i]['wholeseller']."' id='trans_name' class='form-control' required='required'></td>";
                echo "<td class='col-md-1'><button type='button' class='btn btn-success' id='bt_add_new0' onclick='add_new_row($i)'> + </button></td>";
                echo "<td class='col-md-1'><button type='button'id='deleterow' class='btn btn-danger' onclick='delterec(".$prolist['alldata'][$i]['PRO_ID'].",".$prolist['alldata'][$i]['IND_ID'].");'>X</button></td>";                  
                echo "<tr>";
              }
           ...
           //$this->response->redirect($this->url->link('manageorder/orderlist', 'token=' . $this->session->data['token'] . $url, true));
        }
        
        
        //insert Date when User Remove order 
        
        
        public function insertdeletedata()
        {
            
            $this->load->model('manageorder/orderlist');
            
            $data= $this->model_manageorder_orderlist->insertDeleteData($this->request->get['pid'],$this->request->get['oid']);
        ...
        public function  submitData()
        {
            ...
            $data= $this->model_manageorder_orderlist->save($this->request->post);
            //check Data 
            
            if($data==1)
            {
                 $_SESSION['success'] = 'Records SucessFully Updated';
            }
            else 
               {
                    $_SESSION['input_error'] = 'Sorry ! Records Not Updated';
               }
