...
error_reporting(0);
class ControllerReportDailysummaryreport extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Daily Summary');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Sale Summary',
			'href' => $this->url->link('report/dailysummaryreport', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                ...
                $data["listMOs"] = $this->model_report_dailysummaryreport->getMo();
                $data["listDISTs"] = $this->model_report_dailysummaryreport->getdistrict();
                $data["listTEHs"] = $this->model_report_dailysummaryreport->gettehsil();
		...
		$results = $this->model_report_dailysummaryreport->getdailysummary($filter_data);
                //print_r($results);
		...
		$pagination->url = $this->url->link('report/dailysummaryreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/dailysummaryreport.tpl', $data));
	}
...
public function download_excel() {
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['mst_farmer_cnt']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['retailer_cnt']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['vill_mtng_cnt']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['farmer_vist']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['pos_vist']);
