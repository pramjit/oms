...
error_reporting(0);
class ControllerReportFarmdemoreport extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Farm Demo Report');
		...
		if (isset($this->request->get['filter_date_start'])) {
			...
			$filter_date_start = NULL;//date('Y-m-d', strtotime(date('Y') . '-' . date('m') . '-01'));
                ...
                if (isset($this->request->get['fm_id'])) {
			$data['filter_fmid'] = $this->request->get['fm_id'];
                        $data['filter_fmid_nm'] = $this->request->get['fmid_nm'];
		} else {
			$data['filter_fmid'] = '';
                        $data['filter_fmid_nm'] = 'Select Farmer';
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Farm Demo Report',
			'href' => $this->url->link('report/farmdemoreport', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/farmdemoreport');
                //$data["listMOs"] = $this->model_report_farmdemoreport->getMo();
               $data["listFARMs"] = $this->model_report_farmdemoreport->getFarmer();
                $data["listVILs"] = $this->model_report_farmdemoreport->getVillage();
		...
		$data['orders'] = array();
                ...
                //$order_total = $this->model_report_farmdemoreport->getTotalSale($filter_data);
		 if($userid=='56' || $userid=='285')
                {
		$results = $this->model_report_farmdemoreport->getadmindtl($filter_data);
                }
                elseif($usergroupid=="3"){
                    $results = $this->model_report_farmdemoreport->getdailyam($filter_data);
                }
                else{
                    $results = $this->model_report_farmdemoreport->getdailysummary($filter_data);
                }
                //print_r($results);
		...
		$pagination->url = $this->url->link('report/farmdemoreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/farmdemoreport.tpl', $data));
	}
...
public function download_excel() {
    ...
    header('Content-Disposition: attachment;filename="farmdemoreport_'.date('dMy').'.xls"');
