...
error_reporting(0);
class ControllerReportMaterialdispatchsummary extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Material Dispatch Summary Report');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/materialdispatchsummary', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/materialdispatchsummary');
                $data["listMOs"] = $this->model_report_materialdispatchsummary->getMo($usergroupid,$userid);
                $data["listWSs"] = $this->model_report_materialdispatchsummary->getWs();
                $data["listAMs"] = $this->model_report_materialdispatchsummary->getAm();
                $data["listDISTs"] = $this->model_report_materialdispatchsummary->getdistrict();
		...
		$data['orders'] = array();
                ...
                $order_total = $this->model_report_materialdispatchsummary->getTotalSale($filter_data);
               
                $results = $this->model_report_materialdispatchsummary->getdailysummary($filter_data);
		...
		$pagination->url = $this->url->link('report/materialdispatchsummary', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/materialdispatchsummary.tpl', $data));
	}
...
public function download_excel() {
		...
		$results = $this->model_report_materialdispatchsummary->getdownloadexcel($filter_data);
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['SO_NO']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['PRO_NAME']);
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['DISP_DATE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $data['DISP_QTY']);
    ...
    header('Content-Disposition: attachment;filename="Material_Dispatch_Summary_Report_'.date('dMy').'.xls"');
