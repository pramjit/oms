...
error_reporting(0);
class ControllerReportMaterialreceive extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Material Receive report',
			'href' => $this->url->link('report/materialreceive', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/materialreceive');
                $data["listMOs"] = $this->model_report_materialreceive->getMo($usergroupid,$userid);
                $data["listWSs"] = $this->model_report_materialreceive->getWs();
                $data["listAMs"] = $this->model_report_materialreceive->getAm();
                $data["listDISTs"] = $this->model_report_materialreceive->getdistrict();
		...
		$data['orders'] = array();
                ...
                $order_total = $this->model_report_materialreceive->getTotalSale($filter_data);
                if($userid=='56' || $userid=='283')
                {
		$results = $this->model_report_materialreceive->getadmindtl($filter_data);
                }
                elseif($usergroupid=="3"){
                    $results = $this->model_report_materialreceive->getdailyam($filter_data);
                }
                else
                {
                    $results = $this->model_report_materialreceive->getdailysummary($filter_data);
                }                
               // print_r($results);
		...
		$pagination->url = $this->url->link('report/materialreceive', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/materialreceive.tpl', $data));
	}
 ...
 public function getorderdata(){
    ...
    //print_r($orddata);die;
    $orderdata= $this->model_report_materialreceive->getorder($orddata);
    //print_r($orderdata);
    ?>
...
public function download_excel() {
   ...
   // print_r($results); die;
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $rk );
    ...
    $objWriter->save('php://output');
		...
		$results = $this->model_report_materialreceive->getSale_summary($filter_data);
