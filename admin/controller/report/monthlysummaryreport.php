...
error_reporting(0);
class ControllerReportMonthlysummaryreport extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Monthly Summary');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Sale Summary',
			'href' => $this->url->link('report/monthlysummaryreport', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/monthlysummaryreport');
                $data["listMOs"] = $this->model_report_monthlysummaryreport->getMo();
                $data["listDISTs"] = $this->model_report_monthlysummaryreport->getdistrict();
                $data["listTEHs"] = $this->model_report_monthlysummaryreport->gettehsil();
		...
		$data['orders'] = array();
                ...
                $order_total = $this->model_report_monthlysummaryreport->getTotalSale($filter_data);
		$results = $this->model_report_monthlysummaryreport->getdailysummary($filter_data);
                }
		...
		$pagination->url = $this->url->link('report/monthlysummaryreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/monthlysummaryreport.tpl', $data));
	}
 ...
 public function download_excel() {
    ...
    header('Content-Disposition: attachment;filename="monthlysummaryreport'.date('dMy').'.xls"');
    ...
    $objWriter->save('php://output');
		...
		$results = $this->model_report_monthlysummaryreport->getSale_summary($filter_data);
