...
error_reporting(0);
class ControllerReportPlanreport extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Plan Report');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Plan Report',
			'href' => $this->url->link('report/planreport', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/planreport');
                $data["listMOs"] = $this->model_report_planreport->getMo($usergroupid,$userid);
		...
		$data['orders'] = array();
                ...
                $order_total = $this->model_report_planreport->getTotalSale($filter_data);
		
                if($userid=='56' || $userid=='285')
                {
		$results = $this->model_report_planreport->getadmindtl($filter_data);
                }
                elseif($usergroupid=="3"){
                    $results = $this->model_report_planreport->getdailyam($filter_data);
                }
                else{
                    $results = $this->model_report_planreport->getdailysummary($filter_data);
                }
		...
		$pagination->url = $this->url->link('report/planreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/planreport.tpl', $data));
	}
...
public function download_excel() {
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['TEHSIL']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['cr_date']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['APPROVE']);
    ...
    header('Content-Disposition: attachment;filename="planreport_'.date('dMy').'.xls"');
