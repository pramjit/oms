...
error_reporting(0);
class ControllerReportSaleordersummary extends Controller {
	public function index() {
		$this->load->language('report/sale_report');
		$this->load->language('report/sale_order');
		$this->document->setTitle('Sale Order Summary Report');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Sale Order Summary report',
			'href' => $this->url->link('report/saleordersummary', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$this->load->model('setting/store');
                $this->load->model('report/saleordersummary');
                $data["listMOs"] = $this->model_report_saleordersummary->getMo($usergroupid,$userid);
                $data["listWSs"] = $this->model_report_saleordersummary->getWs();
                $data["listAMs"] = $this->model_report_saleordersummary->getAm();
                $data["listDISTs"] = $this->model_report_saleordersummary->getdistrict();
		...
		$data['orders'] = array();
                ...
                $order_total = $this->model_report_saleordersummary->getTotalSale($filter_data);
               
                    $results = $this->model_report_saleordersummary->getdailysummary($filter_data);
		...
		$pagination->url = $this->url->link('report/saleordersummary', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/saleordersummary.tpl', $data));
	}

 public function getorderdata(){
     
    $this->load->model('report/model_report_saleordersummary');    
...
public function download_excel() {
		...
		$results = $this->model_report_saleordersummary->getdownloadexcel($filter_data);
   ...
   print_r($results); die;
    foreach($results as $data)
    {
        ...
        $col = 0;
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $data['STO_NAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['PRO_CODE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['PRO_NAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['SO_NO']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['SO_QTY']);        
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['DISP_QTY']);        
        
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['IND_NO']);
    ...
    header('Content-Disposition: attachment;filename="Sale_Order_Summary_Report_'.date('dMy').'.xls"');
