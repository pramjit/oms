...
error_reporting(0);
class ControllerstockPurchaseOrder extends Controller{
	public function index()
	{
		//set the title of the page
			$this->document->setTitle("Stock Request List");
			...
			$data['breadcrumbs'][] = array(
			'text' => "Stock Request",
			'href' => $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('stock/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('stock/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['filter'] = $this->url->link('stock/purchase_order/filter', 'token=' . $this->session->data['token'] . $url, true);
		/*getting the list of the orders*/
		
		$this->load->model('stock/purchase_order');
		...
		$data['order_list'] = $this->model_stock_purchase_order->getList($start,$limit);
		...
		//getting total orders
		$data['view'] = $this->url->link('stock/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		$data['receive'] = $this->url->link('stock/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		//getting pages
		...
		$pagination->url = $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('stock/purchase_order_list.tpl', $data));
	}
	...
	public function add()
	{
		...
		$url = '';
			...
			$data['action'] = $this->url->link('stock/purchase_order/insert_purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], true)
		);
		...
		/*getting the product options from database*/
		//$this->load->model('stock/supplier');
		//$data['suppliers'] = $this->model_stock_supplier->get_total_suppliers();
...
//stores
            ...
            if(isset($user['user_group_id']) && $user['user_group_id']== $pos_user_group_id )
	{
		$data['stores'] = $this->model_setting_store->getStoreInv($this->user->getStoreId());                
		...
		$this->response->setOutput($this->load->view('stock/purchase_order_form.tpl', $data));
	}
	...
	public function delete()
	{
		...
		$this->load->model('stock/purchase_order');
		$deleted = $this->model_stock_purchase_order->delete($ids);
		if($deleted)
		{
			$_SESSION['delete_success_message'] = "The order successfully deleted";
			$this->response->redirect($this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
		}
		else
		{
	...
	public function insert_purchase_order()
	{
		...
		else
		{
			...
			if(isset($this->request->post['mail_bit']))
			{
				...
				$html = $this->load->view('stock/mail_purchase_order.tpl',$data);
				...
				$mail->Subject    = "Requirement Products";
				...
				$mail->AddAddress("kunwar.rana@unnati.world", "kunwar rana");
	...
	public function view_order_details()
	{
		...
		$data['order_information'] = $this->model_stock_purchase_order->view_order_details($order_id);
		$data['cancel'] = $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_export'] = $this->url->link('stock/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		if(isset($_GET['export']))
		{
			...
			$html = $this->load->view('stock/print_order.tpl',$data);
		...
		{
			$this->response->setOutput($this->load->view('stock/view_order.tpl',$data));
		}
	}
	...
	{
			$this->document->setTitle("Purchase Request List");
...
$log=new Log("filter.log");
$log->write("filter call");
		$this->load->model('stock/purchase_order');
$log->write("filter call 1");
		$total_orders = $this->model_stock_purchase_order->getTotalOrders();
		/*$pagination = new Pagination();
		$pagination->total = $total_orders;
		$pagination->page = $page;
		...
		if(count(array_filter($post,'strlen')) != 0)
		{
			...
			$data['order_list'] = $this->model_stock_purchase_order->filter($filter,$start,$limit);
			if(!$data['order_list'])
			{
			...
			else
			{
				...
				$total_orders = $this->model_stock_purchase_order->filterCount($filter);
			}
		}
		else
		{
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('stock/purchase_order/filter', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	{
		$this->document->setTitle("Purchase Request");
		$order_id = $this->request->get['order_id'];
		...
		$data['action'] = $this->url->link('stock/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		//$this->load->model('stock/supplier');
		//$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
		$this->response->setOutput($this->load->view('stock/receive_order.tpl',$data));
...
$log->write("before check");
		if((count($received_quantities) != count(array_filter($received_quantities))) || (count($prices) != count(array_filter($prices))) || $order_receive_date == '')
		{
			...
			//exit;//echo $order_receive_date;
			$data['action'] = $this->url->link('stock/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('stock/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			//$this->load->model('stock/supplier');
			//$data['suppliers'] = $this->model_stock_supplier->get_total_suppliers();
			//$this->response->redirect($this->url->link('stock/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true));
			$this->response->setOutput($this->load->view('stock/receive_order.tpl',$data));  
		}
		else
		{
$log->write("after check");
			...
			else
			{
				...
				$this->response->redirect($this->url->link('stock/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true));
			}
