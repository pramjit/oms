...
error_reporting(0);
class ControllerSupplychainsupplychainorder extends Controller{
	...
	$data['WSLIST'] = $this->model_supplychain_supplychainorder->getWS();
        $data['SPLIST'] = $this->model_supplychain_supplychainorder->getSP();
        $data['PROLIST'] = $this->model_supplychain_supplychainorder->getPRO();
        
	$data['order_list'] = $this->model_supplychain_supplychainorder->getFilterList();
		...
		//$this->response->setOutput($this->load->view('report/Inventory_report_product_wise.tpl', $data));
                $this->response->setOutput($this->load->view('supplychain/supplychainorder.tpl', $data));
	}
		...
		if (isset($this->request->get['filter_name_sap']) || isset($this->request->get['filter_model'])) {
			...
			$filter_data = array(
				...
				'limit'        => $limit
                        ...
                        $this->load->model('supplychain/supplychainorder');
		        $results = $this->model_supplychain_supplychainorder->getsap($filter_data);
			foreach ($results as $result) { //print_r($result);
				$option_data = array();
		...
		if (isset($this->request->get['filter_name_wh']) || isset($this->request->get['filter_model'])) {
			...
			$filter_data = array(
				...
				'limit'        => $limit
                        ...
                        $this->load->model('supplychain/supplychainorder');
		        $results = $this->model_supplychain_supplychainorder->getwspname($filter_data);
			foreach ($results as $result) { //print_r($result);
				$option_data = array();
	...
	/*-----------------------------insert receive order function ends here-----------------*/
        ...
        { 
            $log=new Log("Disp_Po_".date('Ymd').".log");
            $this->load->model('supplychain/supplychainorder');
            $msgArr= $this->model_supplychain_supplychainorder->sorderdispatch();
            if($msgArr==0){
                $_SESSION['input_error'] = "Update Not Successfully !";
            }else{
                $this->load->model('user/user');
                $user_info = $this->model_user_user->getUser($this->user->getId());
                $suPId=$user_info['user_id'];
                $Indarr=array();
                foreach($msgArr as $mu){
                    array_push($Indarr,$mu['IndId']);
                }
                $Indarr=array_unique($Indarr); // for unique Indent List
                foreach($Indarr as $Ia){
                    $Qty=0;
                    foreach($msgArr as $mx){
                        if($mx['IndId']==$Ia){
                    ...
                    $ArrInd[]=array('IndId' => $Ind, 'DisQty' => $Qty, 'StoId'=> $Sto);
                }
                $vnum       =   $_REQUEST['vnum'];
                if(empty($vnum)){ $vnum='NA'; }
                $dnum       =   $_REQUEST['dnum'];             
                if(empty($dnum)){ $dnum='NA'; }
                foreach($ArrInd as $ma){
                    
                    $IndId  = $ma['IndId'];
                    $IndQty = $ma['DisQty'];
                    $StoId  = $ma['StoId'];
                    $msgDtls= $this->model_supplychain_supplychainorder->msgDtls($IndId);
                    $MoMob='91'.$msgDtls['MO_MOB'];
                    $MoName=$msgDtls['MO_NAME'];
                    $AmMob='91'.$msgDtls['AM_MOB'];
                    $DlMob='91'.$msgDtls['STO_MOB'];
                    $DlName=$msgDtls['STO_NAME'];
                    ...
                    //=============== Inbound Mobile ===============//    
                            ...
                            //$numbers2 = array($InMob_III);
                            $numbers = array($MoMob, $AmMob, $DlMob);
                            $log->write("Number List:");
                            $log->write($numbers);
                            //$numbers = array($InMob_III,$InMob_IV);
                            //$textmsg='Your Order of TotalKgs KGS has been Dispatched. Your Order ID is OrderId .';
                            $textmsg='Your Order ID is OrdId . Order of M/s PartyName of DispQty KGS has been Dispatched with Vehicle No. VehicleNo & Driver No. DriverMob .';
                            
                            $textmsg=str_replace('OrdId',$IndId,$textmsg);
                            $textmsg=str_replace('PartyName',$DlName,$textmsg);
                            $textmsg=str_replace('DispQty',$IndQty,$textmsg);
                            $textmsg=str_replace('VehicleNo',$vnum,$textmsg);
                            $textmsg=str_replace('DriverMob',$dnum,$textmsg);
                            ...
                            $type='SC_DIS_PO';
                            $msgMaster = $this->model_supplychain_supplychainorder->msgMaster($IndId,$type,$suPId,$StoId,$textmsg,$numbers,$responseBody);
                            $log->write("Message2DB:".$msgMaster);	
                    //============================================================================================//
                    
                    
                }
                $_SESSION['delete_success_message'] = "Update Successfully !";
            }
            
           //$this->response->redirect($this->url->link('supplychain/supplychainorder', 'token=' . $this->session->data['token'] . $url, true));
        }
