...
//new
	public function addProduct($data) {
		$this->event->trigger('pre.admin.product.add', $data);
                ...
                $storeprd = $this->model_setting_store->getStores();
		...
		$product_id = $this->db->getLastId();
                ...
                for($z=0; $z<$totst;$z++)
                {
                    $wsp="WS_ST_".$data['SL'][$z]."_PD_";
                    $rtp="RT_ST_".$data['SL'][$z]."_PD_";
                    $ws=$data[$wsp];
                    $rt=$data[$rtp];
                    $sql="insert into oc_product_to_state set product_id ='".$product_id."',state_id='".$data['SL'][$z]."',ws_price='".$ws."',rt_price='".$rt."', cr_date=CURDATE(), act='1' ";
                    $this->db->query($sql);
                }
                //*********************PRODUCT TO STATE END***************************//
	...
	public function editProduct($product_id, $data) {
		$this->event->trigger('pre.admin.product.edit', $data);
                ...
                for($z=0; $z<$totst;$z++)
                {
                    ...
                    //Check Data Update Date Today or Not
                    $chk="select sid from oc_product_to_state where state_id='".$data['SL'][$z]."' and product_id='".$product_id."'";
                    $query = $this->db->query($chk);
                    $log->write($chk);
                    if($query->num_rows)
                    {
                        $chk="select sid from oc_product_to_state where state_id='".$data['SL'][$z]."' and product_id='".$product_id."' and cr_date=CURDATE()";
                        $query = $this->db->query($chk);
                        $wsp="WS_ST_".$data['SL'][$z]."_PD_".$product_id;
                        $rtp="RT_ST_".$data['SL'][$z]."_PD_".$product_id;
                        $ws=$data[$wsp];
                        $rt=$data[$rtp];
                        $log->write("1".$chk);
                        if($query->row['sid'])
                        {    
                            $sql="update oc_product_to_state set ws_price='".$ws."',rt_price='".$rt."' where state_id='".$data['SL'][$z]."' and product_id='".$product_id."' and act=1";
                            $this->db->query($sql);
                            $log->write($sql);
                        }
                        else
                        {
                            $upd= "update oc_product_to_state set act='0' where state_id='".$data['SL'][$z]."' and product_id='".$product_id."'";
                            $this->db->query($upd);
                            $log->write($upd);
                            $sql="insert into oc_product_to_state set product_id ='".$product_id."',state_id='".$data['SL'][$z]."',ws_price='".$ws."',rt_price='".$rt."', cr_date=CURDATE(), act='1' ";
                            $this->db->query($sql);
 ...
 $this->load->model('setting/store');
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', rtlr_price = '" . (float)$data['rtlr_price'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
...
$log=new Log("log");
		//if (isset($data['product_store'])) 
                   // {
			foreach ($storeprd  as $store_id) {
                            $log->write("INSERT INTO " . DB_PREFIX . "product_to_store SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id['store_id'] . "',quantity = '" . (int)$data['quantitystore'.$store_id['store_id']] . "',store_price='".(int)$data['quantitystoreprice'.$store_id['store_id']]."'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_store SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id['store_id'] . "',quantity = '" . (int)$data['quantitystore'.$store_id['store_id']] . "',store_price='".(float)$data['quantitystoreprice'.$store_id['store_id']]."'");
			}
		//}
...
foreach ($query->rows as $result) {
	...
	public function getTotalProductsByLayoutId($layout_id) {
        ...
        public function getState($pid)
        {
           
            if(empty($pid)){$pid=0;}
            $query = $this->db->query("select akgu.sid as 'ST_ID',akgu.name as 'ST_NAME',a.product_id as 'PROD_ID',a.ws_price as 'WS_PRICE',a.rt_price as 'RT_PRICE',a.cr_date as 'CR_DATE' FROM ak_mst_geo_upper as akgu 
left join
(select sid,product_id,state_id,ws_price,rt_price,cr_date from oc_product_to_state opts
where act=1 and product_id='".$pid."')as a on a.state_id = akgu.sid
where type=2 ");
            return $query->rows;
        }
        public function getStatePrice()
        {
            $query = $this->db->query("select sid as 'ST_ID',`name`as 'ST_NAME' from ak_mst_geo_upper where type='2'");
            return $query->rows;
        }
}