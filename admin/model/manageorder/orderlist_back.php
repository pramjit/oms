...
GROUP BY os.product_id, os.sap_ref, os.sap_date");
        ...
        public function getFilterList()
	{
		...
		$query = $this->db->query("select 
                ...
                where opo.order_status=2 group by opo.id");
		return $query->rows;
	}
        public function getProductList($indid){
            $sql="select order_id as 'IND_ID',product_id as 'PRO_ID', store_id as 'STO_ID', `name` as 'PRO_NAME', quantity as 'PRO_QTY' 
                   from oc_po_product where order_id = '".$indid."'";
