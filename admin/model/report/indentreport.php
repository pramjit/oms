...
where PARENT_USER_ID='".$userid."'");          
    ...
    opo.user_id,
    (CASE
WHEN opo.order_status = 1 THEN 'PENDING'
WHEN opo.order_status = 2 THEN 'APPROVED'
WHEN opo.order_status = 3 THEN 'PENDING FOR DISPATCH'
WHEN opo.order_status = 4 THEN 'PARTIAL DISPATCHED'
WHEN opo.order_status = 5 THEN 'HOLD'
WHEN opo.order_status = 6 THEN 'PARTIAL ORDERED'
WHEN opo.order_status = 7 THEN 'REJECTED'
WHEN opo.order_status = 8 THEN 'RECEIVED'
WHEN opo.order_status = 9 THEN 'DISPATCHED'
WHEN opo.order_status = 10 THEN 'NA'
WHEN opo.order_status = 11 THEN 'PARTIAL RECEIVED'
ELSE 'NA'
END) AS 'order_status',
...
where opo.id='$orddata' group by opp.name ");
    ...
    public function getMoAmWs($ordId){
        $query = $this->db->query("SELECT 
            OPO.id AS 'ORD_ID',
            DATE_FORMAT(OPO.order_date,'%d-%m-%Y') AS 'ORD_DT',
            OPO.user_id AS 'ORD_MO',
            CONCAT(MO.firstname,' ',MO.lastname) AS 'MO_NAME',
            CONCAT(AM.firstname,' ',AM.lastname) AS 'AM_NAME',
            OCS.`name` AS 'WS_NAME',
            OPO.store_id AS 'ORD_STO' 
            FROM oc_po_order OPO 
            JOIN oc_user OCU ON(OPO.user_id=OCU.user_id)
            JOIN ak_customer MO ON(OCU.customer_id=MO.customer_id)
            JOIN ak_customer_map MAP ON(MO.customer_id=MAP.CUSTOMER_ID)
            JOIN ak_customer AM ON(MAP.PARENT_USER_ID=AM.customer_id)
            JOIN oc_store OCS ON(OPO.store_id=OCS.store_id)
            WHERE OPO.id='".$ordId."'");
        return $query->row;
    }
...
$sql.=" 
opo.order_date between ifnull(".$strt_date.",opo.order_date) and ifnull(".$end_date.",opo.order_date) group by ou.customer_id,opo.id order by opo.id desc ";
