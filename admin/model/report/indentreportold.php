...
$sql.=" 
opo.order_date between ifnull(".$strt_date.",opo.order_date) and ifnull(".$end_date.",opo.order_date) group by ou.customer_id,opo.id order by opo.order_date ";
...
opo.order_date between ifnull(".$strt_date.",opo.order_date) and ifnull(".$end_date.",opo.order_date) ";
   ...
   /*******************************************************************/     
	...
	public function getTotalSale($data = array()) {
            ...
            $sql="select count(opo.id)
 FROM oc_po_order as opo
left join oc_user as ou on ou.user_id = opo.user_id 
left join oc_store as os on os.store_id = opo.store_id
left join oc_po_product as opp on opp.order_id = opo.id
left join oc_sap_ref as ref on ref.Indent_id = opo.id
left join ak_customer_map as map on map.customer_id= ou.customer_id
left join ak_customer as c1 on c1.customer_id = map.PARENT_USER_ID";
