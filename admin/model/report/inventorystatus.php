<?php
class ModelReportInventorystatus extends Model {
...
where opo.id='$orddata' group by opp.name ");
       ...
       public function getdailysummary($data = array()) {	  				
            ...
            $sql="select opts.product_id as 'STO_PRO_ID',ocp.model as 'STO_PRO_NAME',opts.quantity as 'STO_PRO_QTY', owcd.title as 'STO_PRO_UNIT'
from oc_product_to_store opts
left join oc_product ocp on(opts.product_id=ocp.product_id)
left join oc_weight_class_description owcd on(ocp.weight_class_id=owcd.weight_class_id)
where store_id='$ws_id'";
        ...
        public function getwhdata($data = array()) {	  				
...
where store_id='$ws_id'";
   ...
   /*******************************************************************/     
	...
	public function getTotalSale($data = array()) {
            ...
            $sql="select count(opts.product_id) as 'total' from oc_product_to_store opts
left join oc_product ocp on(opts.product_id=ocp.product_id)
left join oc_weight_class_description owcd on(ocp.weight_class_id=owcd.weight_class_id)
where store_id='$ws_id'";
             if (isset($data['start']) || isset($data['limit'])) {
