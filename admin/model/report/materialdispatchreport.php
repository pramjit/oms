<?php
class ModelReportMaterialdispatchreport extends Model {
...
where opo.id='$orddata' group by opp.name ");
       ...
       public function getdailysummary($data = array()) {	  				
		...
		$sql="select 
                    DATE_FORMAT(ocd.cr_date,'%d-%m-%Y') as 'DISP_DATE',
                    ocd.transport_name as 'DISP_TRANS',
                    ocd.vehicle_no as 'DISP_VEHI',
                    ocd.grr_details as 'DISP_GRR',
                    ocd.driver_mob as 'DISP_MOB',
                    ocd.invoice_no as 'DISP_INV',
                    ocdd.store_id as 'STO_ID',
                    ocs.`name` as 'STO_NAME',
                    ocdd.sap_ref_no as 'ORD_NO',
                    ocd.INVOICE_NO as 'IND_NO',
                    ocdd.ind_qty as 'IND_QTY',
                    ocdd.so_qty as 'SO_QTY',
                    ocdd.disp_qty as 'DISP_QTY'
                    from oc_dispatch ocd
                    left join oc_dispatch_details ocdd on(ocd.dispatch_id=ocdd.dispatch_id)
                    left join oc_store ocs on(ocdd.store_id=ocs.store_id)
                    where ocd.cr_date!=''";
                    
                    if($ws_id!=NULL)
                    {
                    $sql.=" and ocdd.store_id in (".$ws_id.")  ";
                    }
                    elseif($strt_date!=NULL )
                    {
                    $sql.="and DATE(ocd.cr_date) between ifnull(".$strt_date.",ocd.cr_date) and ifnull(".$end_date.",ocd.cr_date)";
                    }
...
opo.order_date between ifnull(".$strt_date.",opo.order_date) and ifnull(".$end_date.",opo.order_date) group by ou.customer_id,opo.id";
   ...
   /*******************************************************************/     
	...
	public function getTotalSale($data = array()) {
		...
		$sql="select count(ocdd.sap_ref_no) as 'total',
                    DATE_FORMAT(ocd.cr_date,'%d-%m-%Y') as 'DISP_DATE',
                    ocd.transport_name as 'DISP_TRANS',
                    ocd.vehicle_no as 'DISP_VEHI',
                    ocd.grr_details as 'DISP_GRR',
