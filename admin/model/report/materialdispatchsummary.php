<?php
class ModelReportMaterialdispatchsummary extends Model {
...
where opo.id='$orddata' group by opp.name ");
       ...
       public function getdailysummary($data = array()) {
		...
		$sql="select 
DATE_FORMAT(owt.sap_date,'%d-%m-%Y') as 'SO_DATE',
owt.sap_ref as 'SO_NO',
owt.prod_id as 'SO_PRO',
IFNULL(opp.so_qty, 0) as 'SO_QTY',
IFNULL(odd.disp_qty, 0) as 'DISP_QTY',
DATE_FORMAT(odd.cr_date,'%d-%m-%Y') as 'DISP_DATE',
ocs.name as 'STO_NAME',
ocp.model as 'PRO_NAME'
from oc_ws_trans owt
left join oc_po_product opp on (owt.ind_no=opp.order_id and owt.prod_id=opp.product_id)
left join oc_dispatch_details odd on (owt.sap_ref = odd.sap_ref_no and owt.prod_id=odd.product_id and owt.ind_no=odd.indent_no)
left join oc_store ocs on (opp.store_id = ocs.store_id)
left join oc_product ocp on(opp.product_id = ocp.product_id) where  owt.sap_date!=''";
          
                    if($ws_id!=NULL)
                    {
                    $sql.=" and opp.store_id in (".$ws_id.")  ";
                    }
                    elseif($strt_date!=NULL )
                    {
                    $sql.="and DATE(owt.sap_date) between ifnull(".$strt_date.",owt.sap_date) and ifnull(".$end_date.",owt.sap_date)";
                    }
