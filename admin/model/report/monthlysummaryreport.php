<?php
class ModelReportMonthlysummaryreport extends Model {
...
where date(CR_DATE)> (NOW() - INTERVAL 1 MONTH)
group by cr_by)as a on a.cr_by = cu.customer_id

left join 

(select cr_by,count(cr_by)as retailer_cnt from ak_mst_retailer
where date(CR_DATE)> (NOW() - INTERVAL 1 MONTH)
 group by cr_by)as b
...
(select user_id,count(*)as vill_mtng_cnt from ak_fgm_dtl
where date(CR_DATE)> (NOW() - INTERVAL 1 MONTH)
group by user_id 
)as c
...
sum(case when VISIT_TYPE=2 then 1 else 0 end)as farmer_vist
 from ak_user_visit where date(CR_DATE)> (NOW() - INTERVAL 1 MONTH)
