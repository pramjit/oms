...
class ModelReportpaymentreport extends Model {
    ...
    public function MsgDtls($payId){
        $log=new Log("ADM_RCV_PAY_MOD".date('Ymd').".log");
		$chkSql="SELECT customer_group_id FROM ak_customer WHERE customer_id IN(SELECT Emp_id FROM oc_payment_ref WHERE Sid='".$payId."')";
		$log->write($chkSql);
                $chkQry = $this->db->query($chkSql);
		if($chkQry->row['customer_group_id']==3){ // Payment Have Submitted BY AM
		$sql="SELECT  
			PAY.Emp_id AS 'MO_ID',PAY.Amnt_Rs AS 'TOT_RS', PAY.Cust_id AS 'STO_ID',
			EMP.MO_NAME,EMP.MO_MOB,EMP.AM_NAME,EMP.AM_MOB,DIS.DL_NAME,DIS.DL_MOB,DIS.DL_DIST
			...
			FROM ak_customer MO 
			) EMP ON(PAY.EMP_id=EMP.MO_ID)

			JOIN(SELECT OC.customer_id AS 'DL_ID',OC.store_id AS 'DL_STO',AK.firstname AS 'DL_NAME',AK.telephone AS 'DL_MOB',MP.GEO_ID AS 'DL_DID', DT.`NAME` AS 'DL_DIST'
			FROM oc_user OC
			JOIN ak_customer AK ON(OC.customer_id=AK.customer_id)
			JOIN ak_customer_emp_map MP ON(OC.customer_id=MP.CUSTOMER_ID AND MP.GEO_LEVEL_ID=4)
                        JOIN ak_mst_geo_upper DT ON(MP.GEO_ID=DT.SID)) DIS ON(PAY.Cust_id=DIS.DL_STO)

			WHERE PAY.Sid='".$payId."'";
		}else{// Payment Have Submitted BY MO
