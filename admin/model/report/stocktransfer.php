<?php
class ModelReportStocktransfer extends Model {
...
where opo.id='$orddata' group by opp.name ");
       ...
       public function getdailysummary($data = array()) {	
         $strt_date=$this->request->get['filter_date_start'];
            ...
            $sql="select oso.id as 'SO_ID', DATE_FORMAT(oso.order_date,'%d-%m-%Y') as 'SO_DATE' ,oso.recipient_number as 'SO_PARTY_NO', amr.retailer_name as 'SO_PARTY_NAME',
osp.product_id as 'OS_PRO_ID', osp.`name` as 'OS_PRO_NAME', osp.quantity as 'OS_PRO_QTY',
REPLACE(osp.price,'Rs. ','') as 'OS_PRO_AMT'
from oc_stock_order oso
left join oc_stock_product osp on (oso.id=osp.order_id)
left join ak_mst_retailer amr on (oso.recipient_number=amr.mobile) where DATE(oso.order_date) between ifnull(".$strt_date.",DATE(oso.order_date)) and ifnull(".$end_date.",DATE(oso.order_date)) ORDER BY oso.order_date desc";
   ...
   /*******************************************************************/     
	...
	public function getTotalSale($data = array()) {
         ...
         $strt_date=$this->request->get['filter_date_start'];
            ...
            $sql="select count(oso.id) as 'total'
from oc_stock_order oso
left join oc_stock_product osp on (oso.id=osp.order_id)
left join ak_mst_retailer amr on (oso.recipient_number=amr.mobile) where DATE(oso.order_date) between ifnull(".$strt_date.",DATE(oso.order_date)) and ifnull(".$end_date.",DATE(oso.order_date)) ORDER BY oso.order_date desc";
