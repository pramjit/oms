...
class ModelSaleOrder extends Model {
    ...
    public function getWs(){
          ...
          $query = $this->db->query("select name,store_id from oc_store where act=1");//where PARENT_USER_ID='12'
          return $query->rows;  
    }
    
    public function getOrdercurmonth($data = array()) 
    {
    ...
    $sql="
SELECT 
SUM(opp.quantity) AS ordered,
SUM(opp.received_products) AS dispatch,
(SUM(opp.quantity) - SUM(opp.received_products)) AS Pending
FROM oc_po_product AS opp
LEFT JOIN oc_user ocu ON (opp.store_id = ocu.store_id)
LEFT JOIN oc_po_order opo ON (opo.id = opp.order_id)
LEFT JOIN ak_customer_emp_map akm ON (ocu.customer_id=akm.CUSTOMER_ID and GEO_LEVEL_ID=4)
where month(opo.order_date)='$curmonth'";
if($dist_id!=NULL)
{
$sql.=" and akm.GEO_ID in (".$dist_id.") ";
}
if($store_id!=NULL)
{
$sql.=" and opp.store_id (".$store_id.") ";
} 
     ...
     public function getOrder3month($data = array()) 
    {
         $dist_id=$this->request->get['dist_id'];
         $store_id=$this->request->get['store_id'];
...
$sql.=" and opp.store_id (".$store_id.") ";
     ...
     public function getOrder2month($data = array()) 
    {
     $dist_id=$this->request->get['dist_id'];
     $store_id=$this->request->get['store_id'];
     
     $curmonth=date('m', strtotime('-2 month'));
     $sql="
SELECT 
SUM(opp.quantity) AS ordered,
SUM(opp.received_products) AS dispatch,
...
$sql.=" and opp.store_id (".$store_id.") ";
     ...
     public function getOrder1month($data = array()) 
    {
     ...
     $curmonth=date('m', strtotime('-3 month'));
...
$sql.=" and opp.store_id (".$store_id.") ";
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$affiliate_  = $this->model_marketing_affiliate->getAffiliate($affiliate_id);
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer,o.telephone, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified,o.payment_method,o.store_name FROM `" . DB_PREFIX . "order` o";
