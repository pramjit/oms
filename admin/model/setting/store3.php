...
class ModelSettingStore extends Model {
	...
	public function getStore($store_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "store WHERE store_id = '" . (int)$store_id . "' AND act=1");
	...
	public function getStoreInv($store_id) {
		...
		//$store_data = $this->cache->get('store');
                $mcrypt = new MCrypt();
                $log=new Log("ADMIN_MODEL_SETTING_STORE_".date('d-m-Y').".log");
                $storeid =$mcrypt->decrypt($this->request->post['store_id']);
                $userid =$mcrypt->decrypt($this->request->post['user_id']);
                $roleid =$mcrypt->decrypt($this->request->post['api_group_id']);
                $keys = array('store_id','user_id','api_group_id');
                $json = array();
                foreach ($keys as $key) {
                ...
                if($roleid==3)
                {
                    ...
                    WHERE akm.GEO_LEVEL_ID = 4 AND akm.GEO_ID IN(
                    select geo_id from ak_customer_emp_map
                    WHERE CUSTOMER_ID IN(select acm.customer_id from ak_customer_map acm
                    left join ak_customer acu on (acm.customer_id = acu.customer_id)
                    where parent_user_id='".$userid."' and acu.customer_group_id=4
                    ) and GEO_LEVEL_ID=4) AND ocu.user_group_id = 10)
					AND act=1
                    ORDER BY url";
                }
                if($roleid==4)
                {
                    ...
                    WHERE akm.GEO_LEVEL_ID = 4 AND akm.GEO_ID IN(
                    select geo_id from ak_customer_emp_map
                    WHERE CUSTOMER_ID = '".$userid."'  and GEO_LEVEL_ID=4) AND ocu.user_group_id = 10)
					AND act=1
                    ORDER BY url";
                }
                if($roleid==10)
                {
                    $sql="SELECT * FROM oc_store 
                    WHERE store_id IN(select store_id from oc_user where customer_id='".$userid."' and user_group_id = 10) AND act=1 ORDER BY url";
                }
				...
				if (!$store_data) {
                    $store_data = array(array());
						...
						*/
						...
						if(empty($query->rows)){
							$query = $this->db->query(" select store_id,`name`,url from oc_store where act=1 limit 1");
						}
						foreach ($query->rows as $storedb) {
							$store_datan =array(
								'store_id' => $storedb['store_id'],
	...
	public function getTotalStores() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "store WHERE act=1");
