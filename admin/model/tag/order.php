<?php
class ModelTagOrder extends Model {
	...
	public function getOrders($data = array()) {
	        $sql = "SELECT o.order_id,o.store_name, CONCAT(o.payment_address_1) AS customer,o.telephone, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_potential FROM `" . DB_PREFIX . "order_leads` o  ";
		...
		if (!empty($data['filter_date_potential'])) {
			$sql .= " AND DATE(o.date_potential) <= DATE('" . $this->db->escape($data['filter_date_potential']) . "')";
		}
	...
	public function getOrderProducts_2($order_id) {
                $sql="SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'";
		$query = $this->db->query($sql);

		return $query->rows;
	}
        public function getOrderOptions_2($order_id, $order_product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product_id . "'");

		return $query->rows;
	}
        public function getOrderVouchers_2($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_voucher WHERE order_id = '" . (int)$order_id . "'");

		return $query->rows;
	}
        public function getOrderTotals_2($order_id) {
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_date_potential'])) {
			$sql .= " AND DATE(date_potential) = DATE('" . $this->db->escape($data['filter_date_potential']) . "')";
		}
		...
		if (!empty($data['filter_total'])) {
			$sql .= " AND total = '" . (float)$data['filter_total'] . "'";
                ...
                $sql.=" AND payment_method='Tagged' ";//echo $sql;
		$query = $this->db->query($sql);
	...
	public function getTotalEmailsByProductsOrdered($products) {
		...
		return $query->row['total'];
              ...
              public function getReqID($order_id) {
		
		$query = $this->db->query("SELECT bill_id FROM `oc_requisition_to_bill`  WHERE `requisition_id`= '".$order_id."'");

		return $query->row['bill_id'];
	}
	public function getOrderDetailsFromOrder($order_id) {
	     $sql="SELECT shipping_address_2,shipping_city FROM `oc_order`  WHERE `order_id`= '".$order_id."'";
		$query = $this->db->query($sql);
