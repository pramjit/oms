<?php
class Controllermposdsclorder extends Controller {
	...
	public function sendorder() {
		//SMS LIB rqid ttp tel
		$log=new Log("smsdscl-".date('Y-m-d').".log");			
		 $mcrypt=new MCrypt();
		...
		$this->load->library('sms');	
	        $sms=new sms($this->registry);
                $sms->sendsms($this->request->post['tel'],"5",$this->request->post);                
	...
	public function addOrder() {
		 $mcrypt=new MCrypt();
		$log=new Log("dsclorderemp-".date('Y-m-d').".log");
		$log->write($this->request->post);
		$keys = array(
			'store_id',
			'payment_method',
	...
	foreach ($keys as $key) {
                ...
                $this->request->post[$key] =trim($mcrypt->decrypt($this->request->post[$key]));
			...
			$customer_id =$this->request->post['customer_id'];
				$gcustomer_id = $this->request->post['customer_id'];
	...
	{
		$this->addcustomer($this->request->post['store_id'],$this->request->post);
		$customer_id=$this->model_pos_pos->getCustomerByPhone($this->request->post['customer_mob'])["customer_id"];
	...
	$log->write("add to cart=".$gcustomer_id);
	...
	//add data to cart
		...
		unset($this->session->data['shipping_method']);                 
                $data = array();                
		$data['gcustomer_id'] = $gcustomer_id;
                //validation 
                $errors = '';
		...
		$data['ename'] = $this->request->post['ename'];
		$data['store_id'] = $this->request->post['store_id'];
		$data['store_name'] = $this->request->post['store_name'];
		$data['village_name']=	$this->request->post['village_name'];
		$data['village_id']=	$this->request->post['village_id'];
		$data['farmername']=$this->request->post['farmer_name'];		
		$data['circle_code']=$this->request->post['shipping_code'];
		...
		$data['payment_tax_id'] = '';
		$data['payment_address_1'] = $gcustomer_id;//$this->request->post['customer_id'];
		...
		$data['shipping_address_1'] = $this->request->post['transid'];
		...
		$data['shipping_code'] = $this->request->post['shipping_code'];//'pickup.pickup';		
		...
		$data['user_id'] =$this->request->post['user_id']; //$this->user->getId();
                $log->write("user id");
		...
		if(isset($customer_id))
		{
		 ...
		 $data['payment_firstname'] = $this->request->post['fname'];
		$data['payment_lastname'] = $this->request->post['farmer_name'];

		$data['shipping_firstname']=	$this->request->post['village_name'];
		...
		$data['total']=	$total;	
		$data['village_id']=	$this->request->post['village_id'];
		$data['transid']= $order_id.$this->request->post['transid'];
		//send data to sugar
		if($payment_method !='Cash')
		{

			$this->model_pos_pos->sendsugar($data);
			try{
			//send sms for rqid
			$this->load->library('sms');	
		            $sms=new sms($this->registry);
	                    $sms->sendsms($data['telephone'],"6",$data);                
			} catch (Exception $e) {
...
public function addcustomer($sid,$data)
                {                                                                      
             $data['card']=="0";   
 if(isset($data['farmer_name']))
	{
             $this->request->post['firstname']==$data['farmer_name'];
	}
	else{
	...
	{
	$this->request->post['village']==$data['village_name'];                             	
	}else{
