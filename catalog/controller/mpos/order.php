...
public function addcustomer($sid)
	...
	public function OrderProducts() 
		...
		$orderlead=$this->model_lead_orderleads->getOrderdtl($oid);
							$log->write("in data s ");
				$log->write($orderlead);
		$data['cus_id']=$mcrypt->encrypt($orderlead['payment_address_1']);
		$data['far_name']=$mcrypt->encrypt($orderlead['payment_firstname']);
		$data['fath_name']=$mcrypt->encrypt($orderlead['payment_lastname']);
		$data['vill_name']=$mcrypt->encrypt($orderlead['shipping_firstname']);
		$data['stor_name']=$mcrypt->encrypt($orderlead['store_name']);
		}catch(Exception $el){
			$log->write($el);
			}
