...
//ak_geo
$file_db->exec("CREATE TABLE IF NOT EXISTS ak_geo (
...
//save data  crop
//find user group from ak_customer
/*$usql="select akc.customer_id as 'urole'
from oc_user ocu
left join ak_customer akc
 on(ocu.username = akc.user_id)
where ocu.user_id =".$sid;
...
where CUSTOMER_ID in($id,(select ac.CUSTOMER_ID FROM ak_customer_map as ac 
JOIN ak_customer as au on au.customer_id=ac.CUSTOMER_ID
where ac.PARENT_USER_ID='".$id."')) and GEO_LEVEL_ID=4 )");
}
 else {
...
foreach ($data as $value){
    $insert="insert into ak_mst_geo_lower (SID,NAME,TYPE,PINCODE,TEHSIL_ID,BLOCK_ID,DISTRICT_ID,STATE_ID,ACT,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["NAME"])."','".$mcrt->encrypt($value["TYPE"])."','".$mcrt->encrypt($value["PINCODE"])."','".$mcrt->encrypt($value["TEHSIL_ID"])."','".$mcrt->encrypt($value["BLOCK_ID"])."','".$mcrt->encrypt($value["DISTRICT_ID"])."','".$mcrt->encrypt($value["STATE_ID"])."','".$mcrt->encrypt($value["ACT"])."','0')";
    //$insert="insert into ak_mst_geo_lower (SID,NAME,TYPE,PINCODE,TEHSIL_ID,BLOCK_ID,DISTRICT_ID,STATE_ID,ACT,statusl) values('".$value["SID"]."','".$value["NAME"]."','".$value["TYPE"]."','".$value["PINCODE"]."','".$value["TEHSIL_ID"]."','".$value["BLOCK_ID"]."','".$value["DISTRICT_ID"]."','".$value["STATE_ID"]."','".$value["ACT"]."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
...
if($CGrp == 3){
$query = $this->db->query("SELECT *  FROM ak_mst_geo_upper where SID in(SELECT geo_id FROM ak_customer_emp_map where CUSTOMER_ID in($id, 
...
//save data ak_farm_obbservation
...
//save data customer
$query = $this->db->query("SELECT customer_id,firstname,lastname  FROM ak_customer");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ak_customer (customer_id,Name,statusl) values('".$mcrt->encrypt($value["customer_id"])."','".$mcrt->encrypt($value["firstname"].' '.$value["lastname"])."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
}
...
$query = $this->db->query("SELECT *  FROM ak_geo ");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ak_geo (SID,GEO_NAME,GEO_TYPE,Nation_ID,ZONE_ID,REGION_ID,STATE_ID,AREA_ID,TERRITORY_ID,DISTRICT_ID,ACT,statusl)". " ". "values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["GEO_NAME"])."','".$mcrt->encrypt($value["GEO_TYPE"])."',". "'".$mcrt->encrypt($value["Nation_ID"])."',". "'".$mcrt->encrypt($value["ZONE_ID"])."',". "'".$mcrt->encrypt($value["REGION_ID"])."',". "'".$mcrt->encrypt($value["STATE_ID"])."',". "'".$mcrt->encrypt($value["AREA_ID"])."',". "'".$mcrt->encrypt($value["TERRITORY_ID"])."',". "'".$mcrt->encrypt($value["DISTRICT_ID"])."',". "'".$mcrt->encrypt($value["ACT"])."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
}

//save data ak_customer_emp_map
...
$data=$query->rows; 
foreach ($data as $value){ 
    $insert="insert into ak_plan (SID,TEHSIL,WHOLE_SELLER,CR_DATE,RETAILER,APP_TR_ID,STATUS,CR_BY,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["TEHSIL"])."','".$mcrt->encrypt($value["WHOLE_SELLER"])."','".$mcrt->encrypt(DATE('Y-m-d',strtotime($value["CR_DATE"])))."','".$mcrt->encrypt($value["RETAILER"])."','".$mcrt->encrypt($value["APP_TR_ID"])."','".$mcrt->encrypt($value["STATUS"])."','".$mcrt->encrypt($value["CR_BY"])."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
}
////////////////////////////////////////JEEP CAMPAIGN END//////////////////////////////////////
